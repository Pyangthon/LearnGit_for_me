...
namespace serialPort_Bord
{
    public partial class Form1 : Form
    {
        ...
        const byte ConfigTableNum   = 0XFB;
        const byte ConfigIPAndPort  = 0XFA;
        const byte GetIPAndPortDet  = 0XFC;
        const byte ReturnSuccess    = 0XFF;
        const byte FrameTail        = 0X16;
        const byte HeadWareDet      = 0XFD;
            ...
            lock(ReceBuffer)        // 锁住该数组，保证数组在使用时不会被再次使用
            {
                ...
                else
                {
                    if (ReceBuffer[9] == ConfigTableNum)      // 修改表号
                    {
                        ...
                        {
                            Dis_Rece_TextBox.AppendText("表号修改成功\r\n");
                    ...
                    else if(ReceBuffer[9] == GSMCMDEnum.readFirmware)//读固件包
                    {
        ...
        private void btnOpenFile_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("获取文件路径出错！");
            }
            //判断文件是否存在
            if (!File.Exists(Form1.romPath))
            {
                MessageBox.Show("在磁盘中没有找到Bin文件！");
                return;
            }
        }
        ...
        {
            //获取启动固件升级数据包
           byte[] sendBytes =  UpdateHelper.getFirmwareUpdate();
            //判断数据是否为空
            if (sendBytes == null)
            {
                return;
        ...
        {
            skinProgressBar1.Maximum = 100;//设置最大长度值           
